...
class VendorLedgerService {
    ...
    async fetchVendorLedger(vendorId: number): Promise<{ data: VendorLedgerEntry[]; error?: string }> {
        try {
            const response = await fetch(
                `${getSupabaseBaseUrl()}/rest/v1/vendor_ledger?vendor_id=eq.${vendorId}&order=transaction_date.desc,ledger_id.desc`,
                {
                    headers: {
    ...
    async getVendorBalance(vendorId: number): Promise<{ data: VendorBalance | null; error?: string }> {
        try {
            const response = await fetch(
                `${getSupabaseBaseUrl()}/rest/v1/vendor_balances?vendor_id=eq.${vendorId}`,
                {
                    headers: {
                        'apikey': getSupabaseAnonKey(),
                        'Authorization': `Bearer ${getSupabaseAnonKey()}`,
    ...
    async getOutstandingInvoices(vendorId: number): Promise<{ data: any[]; error?: string }> {
        try {
            ...
            const purchasesResponse = await fetch(
                `${getSupabaseBaseUrl()}/rest/v1/purchase?vendor_id=eq.${vendorId}&select=purchase_id,invoice_no,amount,invoice_date`,
                {
                    headers: {
                        'apikey': getSupabaseAnonKey(),
                        'Authorization': `Bearer ${getSupabaseAnonKey()}`,
            ...
            const invoicesWithBalance = await Promise.all(
                purchases.map(async (purchase: any) => {
                    const paymentsResponse = await fetch(
                        `${getSupabaseBaseUrl()}/rest/v1/vendor_ledger?vendor_id=eq.${vendorId}&reference_type=eq.PAYMENT&reference_id=eq.${purchase.purchase_id}`,
                        {
                            headers: {
                                'apikey': getSupabaseAnonKey(),
                                'Authorization': `Bearer ${getSupabaseAnonKey()}`,
    ...
    async toggleVendorStatus(vendorId: number, isActive: boolean): Promise<{ success: boolean; error?: string }> {
        try {
            const response = await fetch(
                `${getSupabaseBaseUrl()}/rest/v1/vendor?vendor_id=eq.${vendorId}`,
                {
                    method: 'PATCH',
                    headers: {
                        'apikey': getSupabaseAnonKey(),
