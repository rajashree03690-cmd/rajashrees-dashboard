...
class VendorTransactionService {
    ...
    async fetchVendorTransactions(vendorId: number): Promise<{ data: VendorTransaction[]; error?: string }> {
        try {
            const response = await fetch(
                `${getSupabaseBaseUrl()}/rest/v1/vendor_transactions?vendor_id=eq.${vendorId}&order=transaction_date.desc`,
                {
                    headers: {
    ...
    async fetchUnpaidInvoices(vendorId: number): Promise<{ data: UnpaidInvoice[]; error?: string }> {
        try {
            ...
            const purchasesResponse = await fetch(
                `${getSupabaseBaseUrl()}/rest/v1/purchase?vendor_id=eq.${vendorId}&select=purchase_id,invoice_no,amount`,
                {
                    headers: {
                        'apikey': getSupabaseAnonKey(),
                        'Authorization': `Bearer ${getSupabaseAnonKey()}`,
            ...
            const transactionsResponse = await fetch(
                `${getSupabaseBaseUrl()}/rest/v1/vendor_transactions?vendor_id=eq.${vendorId}&select=purchase_id,amount_paid`,
                {
                    headers: {
                        'apikey': getSupabaseAnonKey(),
                        'Authorization': `Bearer ${getSupabaseAnonKey()}`,
    ...
    async addVendorTransaction(transaction: VendorTransaction): Promise<{ success: boolean; error?: string }> {
        try {
            const response = await fetch(
                `${getSupabaseBaseUrl()}/rest/v1/vendor_transactions`,
                {
                    method: 'POST',
                    headers: {
                        'apikey': getSupabaseAnonKey(),
