...
class ShipmentsApiService {
    ...
    async fetchShipments(): Promise<{ data: Shipment[]; total: number }> {
        try {
            ...
            const response = await fetch(
                `${getSupabaseBaseUrl()}/rest/v1/shipment_tracking?select=*`,
                {
                    headers: {
    ...
    async fetchShipmentByOrderId(orderId: string): Promise<Shipment | null> {
        try {
            ...
            const response = await fetch(
                `${getSupabaseBaseUrl()}/rest/v1/shipment_tracking?order_id=eq.${orderId}&select=*`,
                {
                    headers: {
                        'apikey': getSupabaseAnonKey(),
                        'Authorization': `Bearer ${getSupabaseAnonKey()}`,
    ...
    async fetchTracking(orderId: string): Promise<{ data: any; error: any }> {
        try {
            ...
            const url = `${getSupabaseBaseUrl()}/functions/v1/orders-track?order_id=${orderId}`;
