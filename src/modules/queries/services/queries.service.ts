...
class QueriesService {
    ...
    async fetchQueries(): Promise<{ data: Query[]; error?: string }> {
        try {
            const response = await fetch(
                `${getSupabaseBaseUrl()}/rest/v1/queries?select=*,customers(source,full_name,mobile_number)&order=created_at.desc`,
                {
                    headers: {
    ...
    async addQuery(query: Partial<Query>): Promise<{ success: boolean; error?: string; data?: Query }> {
        try {
            const response = await fetch(
                `${getSupabaseBaseUrl()}/rest/v1/queries`,
                {
                    method: 'POST',
                    headers: {
                        'apikey': getSupabaseAnonKey(),
    ...
    async updateStatus(queryId: number, status: QueryStatus): Promise<{ success: boolean; error?: string }> {
        try {
            const response = await fetch(
                `${getSupabaseBaseUrl()}/rest/v1/queries?query_id=eq.${queryId}`,
                {
                    method: 'PATCH',
                    headers: {
                        'apikey': getSupabaseAnonKey(),
    ...
    async fetchConversations(queryId: number): Promise<{ data: QueryConversation[]; error?: string }> {
        try {
            ...
            const [messagesResponse, conversationsResponse] = await Promise.all([
                fetch(
                    `${getSupabaseBaseUrl()}/rest/v1/query_messages?query_id=eq.${queryId}&order=sent_at.asc`,
                    { headers: { 'apikey': getSupabaseAnonKey(), 'Authorization': `Bearer ${getSupabaseAnonKey()}` } }
                ),
                fetch(
                    `${getSupabaseBaseUrl()}/rest/v1/query_conversations?query_id=eq.${queryId}&order=timestamp.asc`,
                    { headers: { 'apikey': getSupabaseAnonKey(), 'Authorization': `Bearer ${getSupabaseAnonKey()}` } }
                )
            ]);
    ...
    async addConversation(conversation: Partial<QueryConversation>): Promise<{ success: boolean; error?: string }> {
        try {
            ...
            const responseV2 = await fetch(
                `${getSupabaseBaseUrl()}/rest/v1/query_conversations`,
                {
                    method: 'POST',
                    headers: {
                        'apikey': getSupabaseAnonKey(),
            ...
            const responseV1 = await fetch(
                `${getSupabaseBaseUrl()}/rest/v1/query_messages`,
                {
                    method: 'POST',
                    headers: {
                        'apikey': getSupabaseAnonKey(),
