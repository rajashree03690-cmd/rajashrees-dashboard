...
class CallsService {
    ...
    async fetchCalls(filters?: {
        ...
        try {
            let url = `${getSupabaseBaseUrl()}/rest/v1/call_logs?order=started_at.desc`;
    ...
    async fetchExecutiveAvailability(): Promise<{ data: ExecutiveAvailability[]; error?: string }> {
        try {
            const response = await fetch(
                `${getSupabaseBaseUrl()}/rest/v1/executive_availability?select=*,users(user_id,full_name,email)&order=updated_at.desc`,
                {
                    headers: {
                        'apikey': getSupabaseAnonKey(),
                        'Authorization': `Bearer ${getSupabaseAnonKey()}`,
    ...
    async updateExecutiveStatus(executiveId: number, status: ExecutiveStatus): Promise<{ success: boolean; error?: string }> {
        try {
            const response = await fetch(
                `${getSupabaseBaseUrl()}/rest/v1/executive_availability?executive_id=eq.${executiveId}`,
                {
                    method: 'PATCH',
                    headers: {
                        'apikey': getSupabaseAnonKey(),
    ...
    async fetchCallQueue(): Promise<{ data: CallQueue[]; error?: string }> {
        try {
            const response = await fetch(
                `${getSupabaseBaseUrl()}/rest/v1/call_queue?status=eq.waiting&order=queued_at.asc`,
                {
                    headers: {
                        'apikey': getSupabaseAnonKey(),
                        'Authorization': `Bearer ${getSupabaseAnonKey()}`,
    ...
    async fetchCallAnalytics(days: number = 7): Promise<{ data: CallAnalytics[]; error?: string }> {
        try {
            const response = await fetch(
                `${getSupabaseBaseUrl()}/rest/v1/call_analytics?order=call_date.desc&limit=${days}`,
                {
                    headers: {
                        'apikey': getSupabaseAnonKey(),
                        'Authorization': `Bearer ${getSupabaseAnonKey()}`,
