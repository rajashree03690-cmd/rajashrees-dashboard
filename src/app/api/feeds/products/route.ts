...
export const dynamic = 'force-dynamic'; // Prevent Vercel ISR static generation oversized payload error

/**
 * Universal Product Feed API Route
 * 
 * Generates an RSS 2.0 XML feed compatible with:
 * - Google Merchant Center
 * - Facebook/Instagram Product Catalog
 * - Pinterest Shopping
 * 
 * Access: yoursite.com/api/feeds/products
 */
export async function GET() {
    try {
      ...
      `)
        ...
        // Build Universal Product Feed (RSS 2.0)
        ...
        const rssItems = products.map((variant: any) => {
            ...
            // Condition: all products are new
            const condition = 'new';
