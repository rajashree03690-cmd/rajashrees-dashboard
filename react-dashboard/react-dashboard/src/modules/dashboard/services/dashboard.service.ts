...
<<<<<<< HEAD:react-dashboard/react-dashboard/react-dashboard/src/modules/dashboard/services/dashboard.service.ts
            const response = await fetch(
                `${SUPABASE_URL}/functions/v1/products-list?page=1&limit=1`,
                {
                    headers: {
                        'apikey': ANON_KEY,
                        'Authorization': `Bearer ${ANON_KEY}`,
...
=======
            if (error) throw error;
>>>>>>> 49c99a2fbeeeca119250e8d253add75db82d413c:src/modules/dashboard/services/dashboard.service.ts

            console.log(`✅ Product count from Direct DB: ${count}`);
            return count || 0;
        } catch (error) {
