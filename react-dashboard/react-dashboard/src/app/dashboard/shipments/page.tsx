...
import { AddShipmentDialog } from '@/modules/shipments/components/add-shipment-dialog';
...
export default function ShipmentsPage() {
    ...
    const filteredShipments = shipments.filter(s => {
        const matchesSearch =
            ...
            (s.customer_name?.toLowerCase() || '').includes(searchQuery.toLowerCase()); // Added Customer Name search

        const matchesStatus = statusFilter === 'All'
            ? true
            : statusFilter === 'Yet to Ship'
                ? (!s.shipping_status || s.shipping_status === 'Yet to Ship')
                : s.shipping_status === statusFilter;

        const matchesSource = sourceFilter === 'All'
            ? true
            : (s.source || 'Unknown') === sourceFilter;
        ...
        <div className="p-6 space-y-6">
            ...
            <div className="flex flex-col md:flex-row gap-4 items-center justify-between bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
                <div className="flex-1 relative w-full md:w-auto">
                    ...
                    <Input
                        placeholder="Search Order ID, Tracking, or Customer"
                        value={searchQuery}
                        onChange={(e) => {
                ...
                <div className="flex gap-2 w-full md:w-auto">
                    {/* Status Filter */}
                    <select
                        className="px-4 py-2 border rounded-lg bg-white text-sm font-medium focus:ring-2 focus:ring-purple-500 outline-none"
                        value={statusFilter}
                        onChange={(e) => {
