...
export const productsService = {
     ...
     */ async fetchProductsViaEdgeFunction(page: number = 1, limit: number | 'all' = 'all', search?: string): Promise<{
    data: Product[];
    total: number;
}> {
        try {
            ...
            console.log(`✅ Edge Function success! Products: ${result.data?.length || 0}, Total: ${result.total}`);
