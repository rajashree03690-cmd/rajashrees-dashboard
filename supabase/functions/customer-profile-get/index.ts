    ...
    try {
        ...
        if (customerError) {
            ...
            if (customerError.code === 'PGRST116') {
                // 🚀 PREVENT DUPLICATES: Search by email first before creating a new row
                let finalCustomer = null;
                const { data: existingCustomerByEmail, error: emailSearchError } = await supabaseClient
                    .from('customers')
                    .select('*')
                    .eq('email', user.email)
                    .single();

                if (existingCustomerByEmail && !existingCustomerByEmail.auth_id) {
                    // Found a WhatsApp customer matching this email! Link the auth_id.
                    const { data: updatedCustomer, error: linkError } = await supabaseClient
                        .from('customers')
                        .update({ 
                            auth_id: userId,
                            updated_at: new Date().toISOString()
                        })
                        .eq('customer_id', existingCustomerByEmail.customer_id)
                        .select()
                        .single();
                        
                    if (linkError) {
                        console.error('Error linking auth_id to existing customer:', linkError);
                    } else {
                ...
                // Return newly created/linked profile
                return new Response(
                    JSON.stringify({
                        success: true,
                        data: {
                            profile: {
                                ...
                                full_name: finalCustomer.full_name,
                                mobile_number: finalCustomer.mobile_number,
                                address: {
                                    line1: finalCustomer.address,
                                    line2: null,
                                    city: finalCustomer.city,
                                    state: finalCustomer.state,
                                    postal_code: finalCustomer.pincode
                                },
                                created_at: finalCustomer.created_at,
                                updated_at: finalCustomer.updated_at
                            },
                            analytics: {
                                profile_completion: calculateProfileCompletion(finalCustomer),
                                is_new_profile: true,
                                account_age_days: 0
                            }
                        },
                        message: 'Profile linked or created successfully'
                    }),
                    {
                        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
                        status: 200
