...
serve(async (req) => {
    ...
    try {
        ...
        let { data: existingCustomer, error: fetchError } = await supabaseClient
            .from('customers')
            .select('customer_id, email')
            .eq('auth_id', userId)
            .single()

        // 🚀 PREVENT DUPLICATES: If not found by auth_id, check by email
        if (fetchError || !existingCustomer) {
            const { data: emailCustomer } = await supabaseClient
                .from('customers')
                .select('customer_id, email, auth_id')
                .eq('email', user.email)
                .single()

            if (emailCustomer && !emailCustomer.auth_id) {
                // Link the auth_id
                await supabaseClient
                    .from('customers')
                    .update({ auth_id: userId })
                    .eq('customer_id', emailCustomer.customer_id)
