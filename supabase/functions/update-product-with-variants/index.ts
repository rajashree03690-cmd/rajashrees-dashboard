    ...
    try {
        ...
        if (Array.isArray(body.variants)) {
            const variants = body.variants.map((variant: any) => ({
                ...
                sku: variant.sku || `${productUpdate.sku}-${variant.size || 'default'}`,
                saleprice: parseFloat(variant.saleprice ?? variant.salePrice ?? variant.sale_price ?? 0) || 0,
                regularprice: parseFloat(variant.regularprice ?? variant.regularPrice ?? variant.regular_price ?? 0) || 0,
                costprice: parseFloat(variant.costprice ?? variant.costPrice ?? variant.cost_price ?? 0) || null,
