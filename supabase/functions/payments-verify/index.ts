...
/**
 * Payments-Verify Edge Function (Upgraded)
 * 
 * Called by storefront after customer completes Razorpay payment.
 * 
 * Input: { order_id, razorpay_payment_id, razorpay_order_id, razorpay_signature }
 * 
 * Flow:
 * 1. Validate all required fields
 * 2. Verify order exists and check state (idempotency)
 * 3. Verify Razorpay signature (HMAC SHA256)
 * 4. Cross-verify payment amount with Razorpay API
 * 5. Update order: payment_status='paid', order_status='processing'
 * 6. Create shipment_tracking record
 * 7. Clear customer's cart
 * 8. Trigger order confirmation email
 * 9. Log order event
 */
serve(async (req) => {
    ...
    try {
        ...
        if (!order_id || !razorpay_payment_id || !razorpay_order_id || !razorpay_signature) {
            return errorResponse(400, 'Missing payment details: order_id, razorpay_payment_id, razorpay_order_id, razorpay_signature are all required')
        }
        ...
        // ─── 2. Verify Order Exists & Idempotency Check ──────────
        ...
        // ─── 4. Cross-verify payment with Razorpay API ────────────
        ...
        // ─── 6. Create Shipment Tracking Record ──────────────────
        try {
            const { error: trackingErr } = await adminClient.from('shipment_tracking').insert([{
                order_id: updatedOrder.order_id,
                shipping_status: 'Yet to Ship',
                created_at: new Date().toISOString()
            }])
            if (trackingErr) {
                console.warn('⚠️ Shipment tracking insert failed (non-critical):', trackingErr)
            } else {
                console.log('✅ Shipment tracking created for order:', updatedOrder.order_id)
            }
        } catch (trackingError) {
            console.warn('⚠️ Shipment tracking error (non-critical):', trackingError)
        }
        ...
        try {
            ...
            console.log('✅ Triggered order_received email for order:', updatedOrder.order_id)
        } catch (emailError) {
            console.warn('⚠️ Failed to trigger order confirmation email (non-critical):', emailError)
        }
