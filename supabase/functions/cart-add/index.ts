    ...
    try {
        ...
        if (!cart) {
            const { data: newCart, error: cartError } = await supabaseClient
                .from('cart')
                .insert({ customer_id: customer.customer_id, status: 'active' })
                .select('cart_id')
                .single()

            if (cartError || !newCart) throw new Error('Failed to create cart')
            cart = newCart
        }

        // 3. Get variant price from live
        ...
        const finalPrice = variant?.saleprice || variant?.regularprice || 0

        // 4. Check / Update / Insert cart item
